  ...
  <div class="min-h-screen bg-background text-text flex items-center justify-center px-4 py-8">
    <div class="w-full">
      <div class="card auth-card">
        <h1 class="text-center font-bold text-xl mb-3 md:mb-4">Staff Login</h1>
        <p class="text-center text-sm text-text-muted mb-5 md:mb-6">Sign in to your hostel staff account</p>
...
async function requestOtp(isResend = false) {
  ...
  try {
    // First, check if user exists in auth.users or hostel_staff_invites
    const checkRes = await $fetch('/api/auth/check-staff', {
      method: 'POST',
    ...
    if (!checkRes.exists && !checkRes.invited) {
      errorMessage.value =
        'You have not been added as staff to any hostel yet. Please contact your hostel admin.';
      loading.value = false;
      return;
    }
