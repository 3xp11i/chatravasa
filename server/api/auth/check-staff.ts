...
export default defineEventHandler(async (event) => {
  try {
    ...
    // but hostel_staff_invites may have it with '+' (e.g., '+919170147764')
    ...
    // Check if phone exists in hostel_staff_invites (try both formats)
    let invites = [];
    ...
    console.error("Error in check-staff API:", error);
    throw createError({
      statusCode: 500,
